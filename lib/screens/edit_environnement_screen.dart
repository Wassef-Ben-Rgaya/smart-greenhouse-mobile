...
class EditEnvironnementScreen extends StatefulWidget {
  final Environnement environnement;
  final Plant plant;

  const EditEnvironnementScreen({
    super.key,
  ...
  @override
  State<EditEnvironnementScreen> createState() =>
      _EditEnvironnementScreenState();
}

class _EditEnvironnementScreenState extends State<EditEnvironnementScreen> {
  ...
  late double _humidityAirMin;
  late double _humidityAirMax;
  late double? _humiditySoil;
  late int _lightStart;
  late int _lightEnd;
  ...
  void initState() {
    super.initState();
    _tempMin = widget.environnement.temperature.min ?? 18.0;
    _tempMax = widget.environnement.temperature.max ?? 25.0;
    _humidityAirMin = widget.environnement.humidite.air.min ?? 40.0;
    _humidityAirMax = widget.environnement.humidite.air.max ?? 70.0;
    _humiditySoil = widget.environnement.humidite.sol;
    _lightStart = widget.environnement.lumiere.optimalRange.start;
    _lightEnd = widget.environnement.lumiere.optimalRange.end;
    _lightType = widget.environnement.lumiere.type;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Modifier environnement ${widget.plant.nom}'),
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      try {
        final updatedEnvironnement = Environnement(
          id: widget.environnement.id,
          ...
          createdAt: widget.environnement.createdAt,
          updatedAt: DateTime.now(),
        );

        await EnvironnementService().updateEnvironnement(updatedEnvironnement);
        if (!mounted) return;

        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Environnement mis à jour avec succès'),
